...
class ApiService(object):
    ...
    def find_topics_evolution_and_popularity(self):
        ...
        # Visualize topic evolution over time (line chart)
        # for time_interval, topics in topic_evolution.items():
        # plt.plot([time_interval] * len(topics), topics, marker='o', linestyle='', label=str(time_interval))
        ...
        for time_interval, topics in topic_keywords_over_time.items():
            ...
            plt.figure(num=time_interval, figsize=(12, 8))
            plt.bar(topics.values(), topics.values())
            plt.xlabel('Topic ID')
            plt.ylabel('Keywords')
            ...
            plt.show()
        """
        df_keywords = pd.DataFrame(topic_keywords_over_time).T
        plt.figure(figsize=(10, 8))
        sns.heatmap(df_keywords, annot=True, cmap='viridis')
